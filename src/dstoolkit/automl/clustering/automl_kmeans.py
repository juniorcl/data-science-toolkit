...
from dstoolkit.metrics import scores, plots
...
class AutoMLKMeans:
    """
    AutoMLKMeans is a class that automates the process of KMeans clustering model selection
    ...
    def _get_best_params(self, X):
        def objective(trial):
            params = get_kmeans_params_space(trial, self.random_state)
            model = KMeans(**params)
    ...
    def _fit(self, X):
        ...
        self.model = KMeans(**self.best_params)
