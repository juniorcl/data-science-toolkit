...
class AutoMLHistGradientBoosting:
    ...
    def _get_best_params(self):
        def objective(trial):
            params = get_hist_gradient_boosting_params_space(trial, self.random_state)
    ...
    def _fit(self):
        ...
        results = {
            'Train': scores.get_regressor_metrics(self.y_train[self.target], self.y_train['pred']),
            'Valid': scores.get_regressor_metrics(self.y_valid[self.target], self.y_valid['pred']),
            'Test': scores.get_regressor_metrics(self.y_test[self.target], self.y_test['pred']),
        }
